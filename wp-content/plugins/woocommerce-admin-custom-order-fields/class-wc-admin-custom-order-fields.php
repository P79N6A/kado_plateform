...
class WC_Admin_Custom_Order_Fields extends Framework\SV_WC_Plugin {
	...
	public function add_order_details_after_order_table( $order ) {
		...
		$order_fields = $this->get_order_fields( Framework\SV_WC_Order_Compatibility::get_prop( $order, 'id' ), true );
			...
			if ( $order instanceof \WC_Order ) {
				...
				if ( Framework\SV_WC_Plugin_Compatibility::is_wc_version_gte_3_0() ) {
					...
					if ( metadata_exists( 'post', $order_id, $order_field->get_meta_key() ) ) {
						...
						$set_value = true;
						$value     = Framework\SV_WC_Order_Compatibility::get_meta( $order, $order_field->get_meta_key() );
					}
	...
	/**
	 * Returns the plugin documentation url, used for the 'Docs' plugin action.
	 *
	 * @since 1.3.4
	 *
	...
	/**
	 * Returns the support URL, used for the 'Support' plugin action link.
	 *
	 * @since 1.3.4
	 *
