...
class WC_Admin_Custom_Order_Fields_Export_Handler {
	...
	*/
	public function add_fields_to_csv_export_column_data( $order_data, $order, $csv_generator ) {
		...
		foreach ( wc_admin_custom_order_fields()->get_order_fields( Framework\SV_WC_Order_Compatibility::get_prop( $order, 'id' ) ) as $field_id => $field ) {
	...
	protected function get_fields_xml_required_format( $order ) {
		...
		$fields       = array();
		$order_fields = wc_admin_custom_order_fields()->get_order_fields( Framework\SV_WC_Order_Compatibility::get_prop( $order, 'id' ) );
