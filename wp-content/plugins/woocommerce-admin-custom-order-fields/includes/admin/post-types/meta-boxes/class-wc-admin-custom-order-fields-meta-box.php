		...
		foreach ( $order_fields as $custom_field ) {
			...
			if ( '0' === $updated_value || '' === $updated_value || ! empty( $updated_value ) ) {
				...
				if ( 'date' === $order_fields[ $field_id ]->get_type() ) {
					...
					Framework\SV_WC_Order_Compatibility::update_meta_data( $order, $field_meta_key, $order_fields[ $field_id ]->get_value() );

					// This column is used so that date fields can be searchable.
					Framework\SV_WC_Order_Compatibility::update_meta_data( $order, $field_meta_key . '_formatted', $order_fields[ $field_id ]->get_value_formatted() );

				} else {

					Framework\SV_WC_Order_Compatibility::update_meta_data( $order, $field_meta_key, $updated_value );
				}

			// ...Or if it's empty, delete the custom field meta altogether.
			} else {

				Framework\SV_WC_Order_Compatibility::delete_meta_data( $order, $field_meta_key );
				Framework\SV_WC_Order_Compatibility::delete_meta_data( $order, $field_meta_key . '_formatted' );
			}
		}
	}
